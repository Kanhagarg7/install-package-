...
from . import get_string, inline_mention, udB, kanha_bot, kanha_cmd, HNDLR
...
@kanha_cmd(pattern="dsudo( (.*)|$)", fullsudo=True)
async def _(ult):
    inputs = ult.pattern_match.group(1).strip()
    if ult.reply_to_msg_id:
        replied_to = await ult.get_reply_message()
...
from . import get_string, inline_mention, udB, kanha_bot, kanha_cmd, eor, SUDO_HNDLR
